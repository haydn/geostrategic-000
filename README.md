...
decimetre) **squares**. All **players** are all in one game and that is played
...
equipment or infrastructure that can be utilised to progress the game.
...
with a name, icon, size (measured in squares), a flag indicating if it's
autonomous. and a flag indicating if it's a container (can contain other units).
...
units will move with the containing unit. Units cannot move over the top of
units that are smaller in size or not container units.
